...
public class PlanetLander implements Game {
  ...
  public void init() {
	...
	map = new Map("",10f,new Vector2(20,0));
	activeScreen = map;
	isDone = true;
	activeScreen.Init();
    assets().getText("Levels/Sounds.json",
			new Callback<String>() {
				@Override
				public void onSuccess(String result) {
					loadLevels(result);
					assets().getText("Levels/SpriteSheet.json",
							new Callback<String>() {
								@Override
								public void onSuccess(String result) {
	...
	public void LoadEverything(String json) {
		TexturePacker tp = new TexturePacker(watcher,"/images");
